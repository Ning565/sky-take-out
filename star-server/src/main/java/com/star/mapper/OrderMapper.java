...
public interface OrderMapper {
    ...
    @Insert("insert into star_food_chain.orders(number, status, user_id, address_book_id, order_time," +
     ...
     * @param orderNumber
     */
    @Select("select * from star_food_chain.orders where number = #{number}")
    ...
    @Select("select * from star_food_chain.orders where id = #{id}")
    Orders getById(Long id);

    @Select("select count(id) from star_food_chain.orders where status = #{status}") // 计算当前状态的订单数量(即id个数)
     ...
     * @return
     */
    @Select("select * from star_food_chain.orders where status = #{status} and order_time < #{orderTime} ;")
